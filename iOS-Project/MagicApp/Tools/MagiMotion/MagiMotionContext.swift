...
public class MagiMotionContext {
    ...
    internal var viewToTargetState = [UIView: MagiMotionTargetState]()
    ...
    internal var defaultCoordinateSpace = MagiMotionCoordinateSpace.local
...
internal extension MagiMotionContext {
    /**
    ...
    func process(views: [UIView], identifierMap: inout [String: UIView]) {
        for v in views {
            ...
            let targetState: MagiMotionTargetState?
            
            if let modifiers = v.motionModifiers {
                targetState = MagiMotionTargetState(modifiers: modifiers)
...
public extension MagiMotionContext {
    /**
     ...
     */
    subscript(view: UIView) -> MagiMotionTargetState? {
        get {
...
public extension MagiMotionContext {
    ...
    func snapshotView(for view: UIView) -> UIView {
        ...
        let snapshot: UIView
        let snapshotType = self[view]?.snapshotType ?? MagiMotionSnapshotType.optimized
